...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, cashhand, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all CHND specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizecashhandamount=<n>", strprintf(_("Keep N PIV anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Cashhand Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizeCashHandAmount = GetArg("-anonymizecashhandamount", 0);
    if (nAnonymizeCashHandAmount > 999999) nAnonymizeCashHandAmount = 999999;
    if (nAnonymizeCashHandAmount < 2) nAnonymizeCashHandAmount = 2;
    ...
    LogPrintf("Anonymize Cashhand Amount %d\n", nAnonymizeCashHandAmount);
    ...
    /* Denominations
       ...
       For example:
       1CHND+1000 == (.1CHND+100)*10
       10CHND+10000 == (1CHND+1000)*10
    */

...
int64_t GetBlockValue(int nHeight)
{
	...
	// Subsidy to be reviewed according to growth of users

    if (nHeight < 5) {
		nSubsidy = 20000000 * COIN;
    } else if (nHeight > 5 && nHeight < 1001) {
        nSubsidy = 0 * COIN;
	} else if (nHeight >= 1001 && nHeight < 10100) {
		nSubsidy = 50 * COIN;
	} else if (nHeight >= 10100 && nHeight < 20200) {
		nSubsidy = 70 * COIN;
	} else if (nHeight >= 20200 && nHeight < 30300) {
		nSubsidy = 90 * COIN;
	} else if (nHeight >= 30300 && nHeight < 40400) {
		nSubsidy = 110 * COIN;
...
{
    RenameThread("cashhand-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (pindex->nHeight >= 241705 && std::time(nullptr) >= 1540342800 && !IsBlockValueValid(block, nExpectedMint, pindex->nMint)) {
